...
<div id="content"  style="min-height:400px;"> 
  <div class="frametab">
		<h3 style="margin:5px 0 5px 5px;">Kendaraan Rental</h3>
		<button id="add-hotel-tipe">Tambah Kendaraan Rental</button>
...
<script>
	...
	function load_hotel_tipe(){
		...
		$.ajax({
			type : "GET",
			async: false,
			url: '<?php echo base_url();?>index.php/admin/get_assets_rental_vehicle',
			dataType: "json",
			success:function(datajson){
				for(var i=0; i<datajson.length;i++)
					data_via[i] = {number_row: datajson[i].number_row, id:datajson[i].id, vehicle:datajson[i].vehicle, image:datajson[i].image};
			}
		});
			...
			var table = new Y.DataTable({
				columns: [
										...
										
					{
						...
						label: "Ubah",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/rental_vehicle_update/{value}"><img src="<?php echo IMAGES_DIR;?>/edit.ico"/ class="crud-btn"></a>',
						allowHTML: true
					},
					{
						key:"id", 
						label: "Hapus",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/rental_vehicle_delete/{value}" onclick="return deletechecked();" ><img src="<?php echo IMAGES_DIR;?>/delete.ico"/ class="crud-btn"></a>',
						allowHTML: true
					}
				],
				data: data_bank_via,
				caption: "Daftar Kendaraan Rental",
				rowsPerPage: 10
			});
	...
	YUI().use('panel', 'dd-plugin', function (Y) {
		function add_rute(){
			...
			$.ajax({
				type : "GET",
				url: '<?php echo base_url();?>index.php/admin/rental_vehicle_add',
				data: form,
				cache: false,
				dataType: "json",
				success:function(data){
					window.location.assign("<?php echo base_url('index.php/admin/assets_rental/vehicle');?>");
				}
			})
		}
		...
		var panel = new Y.Panel({
			...
			headerContent: 'Tambah Kendaraan Rental',
			width        : 250,
			zIndex       : 5,
			centered     : true,
			modal        : true,
