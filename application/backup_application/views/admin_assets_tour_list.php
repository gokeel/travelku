...
<div id="content"  style="min-height:400px;"> 
  <div class="frametab">
		<h3 style="margin:5px 0 5px 5px;">Tour List</h3>
		<button id="add-hotel-tipe">Tambah Tour List</button>
...
<script>
	...
	function load_hotel_tipe(){
		...
		$.ajax({
			type : "GET",
			async: false,
			url: '<?php echo base_url();?>index.php/admin/get_assets_tour_list',
			dataType: "json",
			success:function(datajson){
				for(var i=0; i<datajson.length;i++)
					data_via[i] = {number_row: datajson[i].number_row, id:datajson[i].id, agent:datajson[i].agent, tour_name:datajson[i].tour_name, category:datajson[i].category, location:datajson[i].location, isactive:datajson[i].isactive};
			}
		});
		...
		YUI({gallery: 'gallery-2013.01.09-23-24'}).use('datatable','datatable-sort','datatype-number','datatype-date','datatable-paginator', function (Y) {
			...
			var table = new Y.DataTable({
				columns: [
					...
					{key:"category", label:"Tour Category"},
					{key:"location", label:"Tour Location"},
										...
										
					{
						...
						label: "Ubah",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/tour_list_update/{value}"><img src="<?php echo IMAGES_DIR;?>/edit.ico"/ class="crud-btn"></a>',
						allowHTML: true
					},
					{
						key:"id", 
						label: "Hapus",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/tour_list_delete/{value}" onclick="return deletechecked();" ><img src="<?php echo IMAGES_DIR;?>/delete.ico"/ class="crud-btn"></a>',
						allowHTML: true
					}
				],
				data: data_bank_via,
	...
	YUI().use('panel', 'dd-plugin', function (Y) {
		function add_rute(){
			...
			$.ajax({
				type : "GET",
				url: '<?php echo base_url();?>index.php/admin/tour_list_add',
				data: form,
				cache: false,
				dataType: "json",
				success:function(data){
					window.location.assign("<?php echo base_url('index.php/admin/assets_tour/list');?>");
				}
			})
		}
		...
		var panel = new Y.Panel({
			...
			headerContent: 'Tambah Tour List',
			width        : 250,
			zIndex       : 5,
			centered     : true,
			modal        : true,
