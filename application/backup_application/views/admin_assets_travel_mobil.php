...
<div id="content"  style="min-height:400px;"> 
  <div class="frametab">
		<h3 style="margin:5px 0 5px 5px;">Travel Mobil</h3>
		<button id="add-hotel-tipe">Tambah Mobil</button>
			...
			<form id="form-add-hotel-tipe" name="form-add-hotel-tipe">
				<fieldset>
					...
					<p>
						...
						<input type="text" name="kapasitas" id="kapasitas" value="" placeholder="">
					</p>
				</fieldset>
			</form>
		</div>
...
<script>
	...
	function load_hotel_tipe(){
		...
		$.ajax({
			type : "GET",
			async: false,
			url: '<?php echo base_url();?>index.php/admin/get_assets_travel_mobil',
			dataType: "json",
			success:function(datajson){
				for(var i=0; i<datajson.length;i++)
					data_via[i] = {number_row: datajson[i].number_row, id:datajson[i].id, mobil:datajson[i].mobil, kapasitas:datajson[i].kapasitas};
			}
		});
			...
			var table = new Y.DataTable({
				columns: [
					...
					{key:"kapasitas", label:"Kapasitas"},										
					{
						...
						label: "Ubah",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/travel_mobil_update/{value}"><img src="<?php echo IMAGES_DIR;?>/edit.ico"/ class="crud-btn"></a>',
						allowHTML: true
					},
					{
						key:"id", 
						label: "Hapus",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/travel_mobil_delete/{value}" onclick="return deletechecked();" ><img src="<?php echo IMAGES_DIR;?>/delete.ico"/ class="crud-btn"></a>',
						allowHTML: true
					}
				],
				data: data_bank_via,
	...
	YUI().use('panel', 'dd-plugin', function (Y) {
		function add_rute(){
			...
			$.ajax({
				type : "GET",
				url: '<?php echo base_url();?>index.php/admin/travel_mobil_add',
				data: form,
				cache: false,
				dataType: "json",
				success:function(data){
					window.location.assign("<?php echo base_url('index.php/admin/assets_travel/mobil');?>");
				}
			})
		}
