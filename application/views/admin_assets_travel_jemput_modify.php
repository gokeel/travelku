  ...
  <!--content--> 
	...
	<div class="frametab">
		<form class="myform" id="edit_form">
			<h3 style="margin:5px 0 5px 5px;">Ubah Data Travel Titik Jemput/h3>
			<table cellpadding="5"  id="table-modify"   cellspacing="0" border="0px" bgcolor="#FFFFFF" class="myTable" >
				...
				<tr class="editTR" >
					<td class="tdTitle">Titik Jemput</td>
					<td><input name="titik_jemput" id="titik_jemput" type="text" value="" size="60"></td>
				</tr>
								
			</table>
			<div id="status"></div>
			<div class="formFooter">
				<input class="mybutton" style="float:left" name="dbproses" type="button" value="Cancel" onclick="document.location.href='<?php echo base_url();?>index.php/admin/assets_travel/jemput';" />
...
<script>
		...
		$.ajax({
			...
			success:function(data){
				for(var i=0; i<data.length;i++){
					...
					$('#titik_jemput').val(data[i].titik_jemput);
				}
			}
		})
	}
	...
	$( window ).load(function() {
	 <?php
		...
		echo 'load_data_room_hotel("'.base_url().'index.php/admin/travel_jemput_details/'.$id.'");';
	 ?>	
	});
	$(document).ready(function() {
		$('#edit_data').click(function(event) {
			...
			$.ajax({
				type : "GET",
				url: "<?php echo base_url().'index.php/admin/travel_jemput_edit/'.$id;?>",
				data: form,
				cache: false,
				dataType: "json",
				success:function(data){
