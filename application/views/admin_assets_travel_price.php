...
<div id="content"  style="min-height:400px;"> 
  <div class="frametab">
		<h3 style="margin:5px 0 5px 5px;">Travel Price</h3>
		<button id="add-hotel-tipe">Tambah Price</button>
		...
		<div class="yui3-widget-bd">
			<form id="form-add-hotel-tipe" name="form-add-hotel-tipe">
				<fieldset>
					...
					<p>
						<label for="chair_type">Chair Type</label><br/>
						<input type="text" name="chair_type" id="chair_type" value="" placeholder="">
					</p>
					<p>
					...
					<p>
						...
						<input type="text" name="isdefault" id="isdefault" value="" placeholder="">
					</p>
				</fieldset>
			</form>
		</div>
...
<script>
	...
	function load_hotel_tipe(){
		...
		$.ajax({
			type : "GET",
			async: false,
			url: '<?php echo base_url();?>index.php/admin/get_assets_travel_price',
			dataType: "json",
			success:function(datajson){
				for(var i=0; i<datajson.length;i++)
					data_via[i] = {number_row: datajson[i].number_row, id:datajson[i].id, trayek:datajson[i].trayek, nama:datajson[i].nama, chair_type:datajson[i].chair_type,agen_price:datajson[i].agen_price, price:datajson[i].price, alot:datajson[i].alot, isdefault:datajson[i].isdefault};
			}
		});
		...
		YUI({gallery: 'gallery-2013.01.09-23-24'}).use('datatable','datatable-sort','datatype-number','datatype-date','datatable-paginator', function (Y) {
			...
			var table = new Y.DataTable({
				columns: [
					...
					{key:"nama", label:"nama"},										
					{key:"chair_type", label:"chair_type"},
					{key:"agen_price", label:"agen_price"},
					...
					{key:"isdefault", label:"isdefault"},
					{
						...
						label: "Ubah",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/travel_price_update/{value}"><img src="<?php echo IMAGES_DIR;?>/edit.ico"/ class="crud-btn"></a>',
						allowHTML: true
					},
					{
						key:"id", 
						label: "Hapus",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/travel_price_delete/{value}" onclick="return deletechecked();" ><img src="<?php echo IMAGES_DIR;?>/delete.ico"/ class="crud-btn"></a>',
						allowHTML: true
					}
				],
				data: data_bank_via,
				caption: "Daftar Price",
				rowsPerPage: 10
			});
	...
	YUI().use('panel', 'dd-plugin', function (Y) {
		function add_rute(){
			...
			$.ajax({
				type : "GET",
				url: '<?php echo base_url();?>index.php/admin/travel_price_add',
				data: form,
				cache: false,
				dataType: "json",
