...
<div id="content"  style="min-height:400px;"> 
  <div class="frametab">
		<h3 style="margin:5px 0 5px 5px;">Umroh Price Default</h3>
...
<script>
	...
	function load_hotel_tipe(){
		...
		$.ajax({
			type : "GET",
			async: false,
			url: '<?php echo base_url();?>index.php/admin/get_assets_umroh_price_default',
			dataType: "json",
			success:function(datajson){
				for(var i=0; i<datajson.length;i++)
					data_via[i] = {number_row: datajson[i].number_row, id:datajson[i].id, agent:datajson[i].agent, umroh_name:datajson[i].umroh_name, agen_price:datajson[i].agen_price, price:datajson[i].price};
			}
		});
			...
			var table = new Y.DataTable({
				columns: [
					...
					{key:"price", label:"Price"},						
					{
						...
						label: "Ubah",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/umroh_price_default_update/{value}"><img src="<?php echo IMAGES_DIR;?>/edit.ico"/ class="crud-btn"></a>',
						allowHTML: true
					},
					{
						key:"id", 
						label: "Hapus",
						width: "30px",
						formatter:'<a href="<?php echo base_url();?>index.php/admin/umroh_price_default_delete/{value}" onclick="return deletechecked();" ><img src="<?php echo IMAGES_DIR;?>/delete.ico"/ class="crud-btn"></a>',
						allowHTML: true
					}
				],
				data: data_bank_via,
				caption: "Umroh Price Default",
				rowsPerPage: 10
			});
	...
	YUI().use('panel', 'dd-plugin', function (Y) {
		function add_rute(){
			...
			$.ajax({
				type : "GET",
				url: '<?php echo base_url();?>index.php/admin/umroh_price_default_add',
				data: form,
				cache: false,
				dataType: "json",
				success:function(data){
					window.location.assign("<?php echo base_url('index.php/admin/assets_umroh/price_default');?>");
				}
			})
		}
