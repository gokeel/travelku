				...
				<fieldset>
					<table>
						...
						<tr>
							...
							<td><input type="radio" name="is_package" value="true">Ya<br><input type="radio" name="is_package" value="false">Tidak</td>
						</tr>
						<tr>
							<td>Dapat dihapus</td>
		...
		$.ajax({
			...
			success:function(datajson){
				for(var i=0; i<datajson.length;i++)
					data[i] = {number_row: datajson[i].number_row, id:datajson[i].id, category:datajson[i].category, description: datajson[i].description, removable: datajson[i].removable, is_package: datajson[i].is_package};
			}
		});
		...
		YUI({gallery: 'gallery-2013.01.09-23-24'}).use('datatable','datatable-sort','datatype-number','datatype-date','datatable-paginator', function (Y) {
			...
			var table = new Y.DataTable({
				columns: [
					...
					{key:"is_package", label:"Berisi Paket?"},
					{
						label:"Hapus",
